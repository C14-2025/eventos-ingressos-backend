...
describe('UpdateEventController', (): void => {
  ...
  it('Should be able to create a new event', async (): Promise<void> => {
    const eventResponse = await request(app.server).post('/events').send({
      title: 'Test Event',
      file_id: id,
      date: '2025-09-20',
      description: 'teste',
    ...
    const event_id = eventResponse.body.data.id

    const response = await request(app.server).put(`/events/${event_id}`).send({
      time: '19:00',
    });
