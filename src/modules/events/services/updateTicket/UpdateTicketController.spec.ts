...
describe('UpdateTicketController', (): void => {
  ...
  it('Should be able to update ticket', async (): Promise<void> => {
    const ticketResponse = await request(app.server).post('/events/sell-ticket').send({
      document: "106614626842",
      event_id: event_id
    });

    const ticket_id = ticketResponse.body.data.id

    const response = await request(app.server).put(`/tickets/${ticket_id}`).send({
      check: true
    });
